...
export const CONTRACT_ADDRESS =
  "CCFCGOSY2YSLLLQ24R5275TAJV4MVNMBLN6Q562HP27HBO6ZVCKWLIZ5";
...
// ============================================================
// Token Farming Platform — Contract Methods
// ============================================================

/**
 * Initialize the farming contract (admin only).
 * Calls: initialize(admin: Address, staking_token: Address, reward_token: Address)
 */
export async function initialize(
  ...
  return callContract(
    ...
    [
      ...
      toScValAddress(stakingToken),
      toScValAddress(rewardToken),
    ],
    caller,
    true
  );
...
/**
 * Stake tokens into the farming contract.
 * Calls: stake(user: Address, amount: i128)
 * Note: User must approve the contract to spend tokens first
 */
export async function stake(
  ...
  return callContract(
    "stake",
    [toScValAddress(user), toScValI128(amount)],
    caller,
    true
  );
}
...
/**
 * Withdraw staked tokens from the farming contract.
 ...
 * Calls: claim(user: Address)
 * Note: Rewards are calculated as 10% of staked amount
 */
